...
(function(mod) {
  ...
  function searchRegexpForward(doc, regexp, start) {
    ...
    for (var line = start.line,
        ch = start.ch,
        last = doc.lastLine(); line <= last; line++, ch = 0) {
      regexp.lastIndex = ch;
  ...
  function searchRegexpBackward(doc, regexp, start) {
    ...
    for (var line = start.line,
        ch = start.ch,
        first = doc.firstLine(); line >= first; line--, ch = -1) {
      var string = doc.getLine(line);
      if (ch > -1)
  ...
  function searchRegexpBackwardMultiline(doc, regexp, start) {
    ...
    var string,
        chunk = 1;
    for (var line = start.line,
        first = doc.firstLine(); line >= first; ) {
      for (var i = 0; i < chunk; i++) {
        var curLine = doc.getLine(line--);
    ...
    search: for (var line = start.line,
      ...
      if (lines.length == 1) {
        ...
        if (string.slice(0, lastLine.length) != lastLine)
          continue search;
        for (var i = 1,
            start = line - lines.length + 1; i < lines.length - 1; i++)
